...
export function computeCurationRankings(survey, responses) {
  ...
  let ranking = survey.items.map((item) => {
    ...
    return {
      ...
      name: item.name,
      photo: item.photo,
      photos: item.photos && item.photos.length ? item.photos : item.photo ? [item.photo] : [],
      productUrl: item.productUrl || "",
      approvedForOrder: Boolean(item.approvedForOrder),
      importedOrderIds: item.importedOrderIds || [],
      votos: votes,
      pctSeleccion: pct,
